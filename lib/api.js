...
api.get_years = function(db, cb) {
  db.collection('guesses').find().toArray(function(e, d) {
    ...
    async.each(d, 
      function(user, cb1) {
        async.each(Object.keys(user.guesses),
          function(city, cb2) {
            async.each(Object.keys(user.guesses[city]),
              function(year, cb3) {
                years_obj[year] = true;
                cb3();
              },
          ...
          function(city, cb2) {
            async.each(Object.keys(user.guesses[city]),
              function(year, cb3) {
                ...
                guess.year = year;
                guess.city = city;
                guess.date = user.guesses[city][year];
                guesses.push(guess);
                cb3();
              },
              function(err) {
...
api.upsert_guess = function(req, res, db, cb) {
  var name = req.body.name.toLowerCase(),
      ...
      year = req.body.year,
      date = req.body.date,
      q = { 'name': name, 'guesses.city': city, 'guesses.year': year },
      u = { $set: { 'guesses.$.date': new Date(date) } },
      opts = { 'upsert': true };
